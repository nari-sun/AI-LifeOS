...
# Use the repository-only permission profile from .codex/config.toml. Do not
# pass --sandbox here: legacy sandbox flags override permission profiles and
# make the repository-only rules ineffective. Git metadata stays protected,
# while on-request approval lets Codex ask before running git/gh operations
# that need .git writes or GitHub network access.
$fixedCodexArgs = @(
    "--ask-for-approval",
    "on-request"
)
