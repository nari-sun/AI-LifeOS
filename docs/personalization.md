...
Chat GUIの入力欄横にある「過去の会話をすべて参照」は、過去チャット検索がONのときだけ使える送信単位の指定です。チェックした送信では通常の上位候補検索だけで終えず、対象となる過去チャットを全件ページングして確認します。現在のliveセッション、一時チャット、記憶除外セッションは従来どおり対象外で、project scopeがある場合はその境界内の全件だけを確認します。チェック状態は会話本文やlive JSONLへ保存しません。
