...
Chat GUIの「過去の会話をすべて参照」をONにした送信だけ、会話アシスタントは通常の上位候補検索ではなく全件確認を実行します。入力文の「全部」「全件」「すべて」などの文言からは自動判定しません。ONの場合は`list_past_chat_sources` を `next_cursor` がなくなるまで呼び、返されたすべての確定済み raw 会話と対象となる未整理 live 会話を `read_past_chat_page` の `next_cursor` がなくなるまで順番に読みます。各ページは連続した文字位置を返すため、長い会話も途中で省略せず確認できます。
